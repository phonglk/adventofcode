...
function run(input, state = 0) {
  ...
  if (op === 99) return input
  const opf = op === 1 
    ? add : op === 2 
      ? multiply 
      : () => { throw new Error(`Invalid op ${op} @ state=${state}`)}
  const posA = input[state+1]
  const posB = input[state+2]
  const posC = input[state+3]

  const c = opf(input[posA], input[posB])
  input[posC] = c

  return run(input, state + 4)
}
...
const check = (a, b) => {
  ...
  clonedInput[1] = a
  clonedInput[2] = b
  return run(clonedInput)[0]
}

for(let i = 0; i < 100; i++) {
  for(let j = 0; j < 100; j++) {
    try {
      if (check(i, j) === 19690720) {
        console.log(i, j)
        break;
      }
    } catch (e) {
